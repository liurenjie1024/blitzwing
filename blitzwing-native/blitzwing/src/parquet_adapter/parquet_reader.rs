use crate::{
  error::{
    BlitzwingErrorKind::{ArrowError, InvalidArgumentError},
    Result,
  },
  ...
  pub(crate) fn next_batch(&mut self) -> Result<&RecordBatch> {
    match &mut self.record_batch {
      ...
      None => {
        ...
        self
          .record_batch
          .replace(RecordBatch::try_new(self.schema.clone(), columns).context(ArrowError)?);
      }
    }
...
pub(crate) fn create_parquet_reader(meta: ParquetReaderProto) -> Result<ParquetReader> {
  ...
  for column in meta.get_column_desc() {
    if let Some((_, field)) = schema.column_with_name(column.get_column_name()) {
      ...
      let array_reader: ArrayReaderRef =
        match (field.data_type(), column.get_physical_type().try_into()?) {
          (&DataType::Int8, Type::INT32) => {
            Box::new(Int8ArrayReader::new(batch_size, true, column_desc_ptr, page_reader_iter)?)
          }
          (&DataType::Int16, Type::INT32) => {
            Box::new(Int16ArrayReader::new(batch_size, true, column_desc_ptr, page_reader_iter)?)
          }
          (&DataType::Int32, Type::INT32) => {
            Box::new(Int32ArrayReader::new(batch_size, false, column_desc_ptr, page_reader_iter)?)
          }
          (&DataType::UInt8, Type::INT32) => {
            Box::new(UInt8ArrayReader::new(batch_size, true, column_desc_ptr, page_reader_iter)?)
          }
          (&DataType::UInt16, Type::INT32) => {
            Box::new(UInt16ArrayReader::new(batch_size, true, column_desc_ptr, page_reader_iter)?)
          }
          (&DataType::UInt32, Type::INT32) => {
            Box::new(UInt32ArrayReader::new(batch_size, false, column_desc_ptr, page_reader_iter)?)
          }
          (&DataType::Int64, Type::INT64) => {
            Box::new(Int64ArrayReader::new(batch_size, false, column_desc_ptr, page_reader_iter)?)
          }
          (&DataType::UInt64, Type::INT64) => {
            Box::new(UInt64ArrayReader::new(batch_size, false, column_desc_ptr, page_reader_iter)?)
          }
          (&DataType::Float32, Type::FLOAT) => {
            Box::new(Float32ArrayReader::new(batch_size, false, column_desc_ptr, page_reader_iter)?)
          }
          (&DataType::Float64, Type::DOUBLE) => {
            Box::new(Float64ArrayReader::new(batch_size, false, column_desc_ptr, page_reader_iter)?)
          }
      ...
      let column_info = ColumnInfo { array_reader, page_readers, column_desc };
  ...
  Ok(ParquetReader { schema: Arc::new(schema), columns, record_batch: None, meta })
}
