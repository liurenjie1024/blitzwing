use crate::{
  error::{HdfsLibError, Result},
  fs::path::{FsPath, FsPathBuilder},
  hadoop_proto::hdfs::{HdfsFileStatusProto, HdfsFileStatusProto_FileType},
};
...
impl<'a> BuildArgs<'a> {
  pub fn new(proto: &'a HdfsFileStatusProto, base_uri: &'a FsPath, parent_path: &'a str) -> Self {
    Self { proto, base_uri, parent_path }
  }
...
impl<'a> TryFrom<BuildArgs<'a>> for FileInfo {
  ...
  fn try_from(value: BuildArgs<'a>) -> Result<Self> {
    Ok(FileInfo {
      ...
      length: value.proto.get_length() as usize,
      block_replication: value.proto.get_block_replication() as u16,
      block_size: value.proto.get_blocksize() as usize,
...
impl<'a> TryFrom<BuildArgs<'a>> for DirInfo {
  ...
  fn try_from(value: BuildArgs<'a>) -> Result<Self> {
    Ok(DirInfo { path: value.build_full_path()?, file_stat: FileStat::try_from(value)? })
  }
}
