...
use crate::error::HdfsLibErrorKind::{FromUtf8Error, PathError};
use url::Url;
use std::sync::Arc;
...
impl FsPathBuilder {
    pub fn new(base: &str) -> Result<Self> {
        Ok(Self {
            url: Url::parse(base).context(PathError)?
        })
    }
    
    pub fn append<B: AsRef<[u8]>>(self, bytes: B) -> Result<Self> {
        let new_url = self.url.join(from_utf8(bytes.as_ref()).context(FromUtf8Error)?)
            .context(PathError)?;
        Ok(Self { url: new_url })
    }
