...
pub fn parse_varint64<R: Read>(input: &mut R) -> Result<u64> {
  ...
  loop {
    ...
    input.read_exact(buf.as_mut()).context(HdfsLibErrorKind::IoError)?;
    let b = buf[0];
...
pub fn parse_delimited_message<R: Read, M: Message>(input: &mut R) -> Result<M> {
  ...
  input.read_exact(&mut buffer).context(HdfsLibErrorKind::IoError)?;

  Ok(parse_from_bytes(&buffer).context(HdfsLibErrorKind::ProtobufError)?)
}
