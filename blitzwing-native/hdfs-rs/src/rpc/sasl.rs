use crate::{
  error::{HdfsLibErrorKind::SaslError, Result},
  hadoop_proto::RpcHeader::{
    RpcRequestHeaderProto, RpcRequestHeaderProto_OperationProto::RPC_FINAL_PACKET, RpcSaslProto,
    RpcSaslProto_SaslAuth, RpcSaslProto_SaslState,
  },
  rpc::{
    auth::{AuthMethod, AuthMethod::Simple, AuthProtocol::Sasl},
    constants::RPC_INVALID_RETRY_COUNT,
    message::{make_rpc_request_header, receive_rpc_response, send_rpc_request},
    user::SubjectRef,
  },
};
use failure::ResultExt;
use gsasl_rs::client::{GssApiInfo, SaslClient};
use tokio::io::{AsyncReadExt, AsyncWriteExt};

lazy_static! {
  static ref SASL_HEADER: RpcRequestHeaderProto =
    make_rpc_request_header(Sasl.call_id(), RPC_INVALID_RETRY_COUNT, RPC_FINAL_PACKET, "");
}
#[derive(new)]
...
impl SaslProtocol {
    ...
    S: AsyncReadExt + AsyncWriteExt + Unpin,
  {
    ...
    loop {
      ...
      let response = receive_rpc_response(stream).await?.get_message::<RpcSaslProto>()?;
        ...
        RpcSaslProto_SaslState::NEGOTIATE => {
          ...
          if let Some(mut sasl_client) = sasl_client_opt {
            let reply_token = if auth_type.has_challenge() {
              ...
              sasl_client.evaluate(auth_type.get_challenge()).context(SaslError)?
            } else if sasl_client.sends_data_first() {
              // Client sends data first
              sasl_client.evaluate(&[]).context(SaslError)?
            } else {
              Vec::new()
            };
            ...
            request_body = SaslProtocol::create_sasl_reply(
              sasl_auth,
        ...
        RpcSaslProto_SaslState::CHALLENGE => {
          if let Some(ref mut sasl_client) = sasl_client_ret {
            check_protocol_content!(
              response.has_token(),
              "Sasl server sent challenge response without token: {:?}",
              response
            );
  ...
  fn create_sasl_reply(
    auth_type: RpcSaslProto_SaslAuth,
    state: RpcSaslProto_SaslState,
  ...
  fn select_sasl_auth<'a>(
    &self,
    auth_types: &'a [RpcSaslProto_SaslAuth],
  ) -> Result<(&'a RpcSaslProto_SaslAuth, Option<SaslClient>)> {
    let mut sasl_client_ret = None;
      ...
      match AuthMethod::value_of(auth_type.get_method()) {
        Ok(auth_method) => {
          if auth_method.mechanism_name() != auth_type.get_mechanism() {
            warn!(
              "Auth method {:?} mechanism not match in server[{}] and client[{}]",
              auth_method,
              auth_type.get_mechanism(),
              auth_method.mechanism_name()
            );
            continue;
          }
          ...
          match self.create_sasl_client(auth_type) {
            ...
            Ok(None) => {
              info!(
                "Can't create sasl client for auth type {:?}, will try next auth type!",
                auth_type
              );
            }
            Err(e) => {
              info!(
                "Failed to create sasl client for auth type {:?}: {}, will try next auth type!",
                auth_type, e
              );
            }
          }
  ...
  fn create_sasl_client(&self, auth_type: &RpcSaslProto_SaslAuth) -> Result<Option<SaslClient>> {
    ...
    match auth_method {
      AuthMethod::Kerberos => self.create_gssapi_sasl_client(auth_type).map(|c| Some(c)),
      _ => Ok(None),
    }
  }

  fn create_gssapi_sasl_client(&self, auth_type: &RpcSaslProto_SaslAuth) -> Result<SaslClient> {
    let gss_api_info = GssApiInfo::new(
      self.subject.fullname().to_string(),
      auth_type.get_protocol().to_string(),
      auth_type.get_serverId().to_string(),
    );

    Ok(SaslClient::use_gss_api(gss_api_info).context(SaslError)?)
  }
}
