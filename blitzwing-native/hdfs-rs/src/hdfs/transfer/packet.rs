use crate::{
  error::{HdfsLibErrorKind, HdfsLibErrorKind::ProtobufError, Result},
  hadoop_proto::datatransfer::PacketHeaderProto,
};
use bytes::Buf;
...
impl PacketHeader {
  pub fn offset_in_block(&self) -> usize {
    self.proto.get_offsetInBlock() as usize
  }
  ...
  pub fn data_len(&self) -> usize {
    self.proto.get_dataLen() as usize
  }
}
...
impl PacketReceiver {
  pub fn new() -> Self {
    Self { buffer: Vec::with_capacity(PACKET_LENGTHS_LEN), cur_packet_info: None }
  }
  ...
  pub fn receive_next_packet<R: Read>(&mut self, stream: &mut R) -> Result<()> {
    ...
    stream.read_exact(&mut self.buffer).context(HdfsLibErrorKind::IoError)?;
    ...
    stream.read_exact(&mut self.buffer[PACKET_LENGTHS_LEN..]).context(HdfsLibErrorKind::IoError)?;
...
pub(crate) mod tests {
  ...
  use crate::{
    ...
    hadoop_proto::datatransfer::PacketHeaderProto,
    hdfs::transfer::packet::{Packet, PacketHeader, PacketInfo, BODY_LENGTH_LEN},
  };
  use bytes::BufMut;
  use failure::ResultExt;
  ...
  impl TestPacketGenerator {
    pub fn generate(&self) -> TestPackets {
      ...
      TestPackets { all_packet_data, packets }
    }
  }
  ...
  fn generate_test_packet(
    ...
    TestPacketData {
      packet_data,
      packet_info: PacketInfo {
        header: PacketHeader { proto: header, payload_len: (checksum.len() + data.len()) as u32 },
  ...
  fn test_read_packet() {
    ...
    for i in 0..test_packet_num {
      ...
      assert_eq!(Some(test_packets.packets[i].packet()), packet_receiver.current_packet());
    }
  }
}
